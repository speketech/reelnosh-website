...
export const LikeButton: React.FC<LikeButtonProps> = ({ 
  ...
  const triggerFeedback = (message: string) => {
    ...
    setFeedback(message);
    // Expand container to reveal feedback, then collapse back after 5 seconds
  ...
  const handleLike = async (e: React.MouseEvent) => {
    ...
    if (currentlyLiked) {
      ...
      triggerFeedback('Thanks for reading either way.');
    } else {
      ...
      triggerFeedback('Thanks, glad it resonated.');
    }
  ...
  if (isFloating) {
    return (
      <div
        className={`inline-flex items-center rounded-full bg-black/55 backdrop-blur-md px-3 py-1.5 border border-white/20 shadow-sm transition-all duration-300 ease-out max-w-full ${className}`}
        onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
        }}
      >
        <button
          ...
          className="relative inline-flex items-center gap-1.5 focus:outline-none cursor-pointer"
          aria-label={isLiked ? 'Unlike note' : 'Like note'}
        >
          <div className={`relative flex items-center justify-center w-5 h-5 transition-transform duration-200 ${isAnimating ? 'scale-125' : (isHovered && !isLiked ? 'scale-110' : 'scale-100')}`}>
          ...
          {displayCount !== null && (
            <span className={`text-[13px] font-medium transition-colors ${isLiked ? 'text-[#D79A8A]' : 'text-[#FFFEFA]'}`}>
              {displayCount}
            </span>
          )}
        </button>

        {/* Feedback inside the container — expands to reveal text, resets after 5s */}
        <div
          className={`overflow-hidden transition-all duration-300 ease-out flex items-center ${
            feedback ? 'max-w-[260px] opacity-100 ml-2 pl-2 border-l border-white/25' : 'max-w-0 opacity-0 ml-0 border-l-0'
          }`}
        >
          <span className="whitespace-nowrap text-[12px] font-medium text-[#FFFEFA] select-none">
            {feedback}
          </span>
        </div>
      </div>
  ...
  // Default variant (e.g. Note Detail page)
  return (
    <div className={`relative inline-flex items-center gap-3 flex-wrap ${className}`}>
      <button
        type="button"
        onClick={handleLike}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        className="relative inline-flex items-center gap-2 focus:outline-none cursor-pointer"
        aria-label={isLiked ? 'Unlike note' : 'Like note'}
      >
        <div className={`relative flex items-center justify-center w-6 h-6 transition-transform duration-200 ${isAnimating ? 'scale-125' : (isHovered && !isLiked ? 'scale-110' : 'scale-100')}`}>
          <svg width="100%" height="100%" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
            {isLiked ? (
              <path d="M30 12.75C30 21.5 17.0262 28.5825 16.4737 28.875C16.3281 28.9533 16.1654 28.9943 16 28.9943C15.8346 28.9943 15.6719 28.9533 15.5262 28.875C14.9737 28.5825 2 21.5 2 12.75C2.00232 10.6953 2.81958 8.72539 4.27248 7.27248C5.72539 5.81958 7.69528 5.00232 9.75 5C12.3312 5 14.5912 6.11 16 7.98625C17.4088 6.11 19.6688 5 22.25 5C24.3047 5.00232 26.2746 5.81958 27.7275 7.27248C29.1804 8.72539 29.9977 10.6953 30 12.75Z" fill={iconColor}/>
            ) : (
        ...
        {displayCount !== null && (
          <span className={`text-[14px] font-semibold transition-colors ${isLiked ? 'text-[#8B3A2A] dark:text-[#D79A8A]' : 'text-neutral-clayGray hover:text-neutral-charcoal'}`}>
            {displayCount}
          </span>
        )}
      </button>

      {/* Feedback pill — design guide tokens (note details page only) */}
      {feedback && (
        <span
          role="status"
          aria-live="polite"
          className="inline-flex items-center rounded-full bg-rn-surface dark:bg-rn-raised px-3.5 py-1.5 font-sans text-xs font-semibold text-rn-text-primary border border-rn-border dark:border-rn-border-strong shadow-rn-soft transition-all duration-200"
        >
          {feedback}
        </span>
      )}
