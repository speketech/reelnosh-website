    ...
    <div className="mx-auto max-w-[1120px] px-5 md:px-12 min-[1120px]:px-0">
      <div className="flex flex-col justify-between gap-8 pb-12 md:flex-row md:items-start">
        <div className="space-y-4">
          <Link href="/" className="block relative h-8 w-32 sm:h-9 sm:w-36" aria-label="Reelnosh Home">
        ...
        <nav className="flex flex-wrap items-center gap-6 text-sm text-white/90" aria-label="Footer navigation">
        ...
        <div className="flex items-center gap-6">
          {socialLinks.map(({ icon, href, label }) => (
            <a key={icon} href={href} target="_blank" rel="noopener noreferrer" className="text-white/80 transition-opacity hover:opacity-70" aria-label={label}>
              <Image src={`/icons/${icon}.svg`} alt="" width={20} height={20} className="h-5 w-5" />
            </a>
          ))}
        </div>
      </div>
