            ...
            return (
              <div key={item.question} className="border-b border-neutral-lightClay dark:border-[#4A352E] py-5 sm:py-6">
                <button type="button" aria-expanded={isOpen} onClick={() => setOpenIndex(isOpen ? null : index)} className="flex w-full items-start justify-between gap-6 text-left group">
                  <span className="font-serif text-xl font-medium leading-snug text-neutral-charcoal dark:text-[#FFF5FA] group-hover:text-clay dark:group-hover:text-[#F4C16D] transition-colors">{item.question}</span>
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#F9F3F1] border border-[#E7DED5] text-[#8B3A2A] dark:bg-[#3C2B25] dark:border-[#4A352E] dark:text-[#F4C16D] font-sans text-lg font-semibold leading-none transition-colors" aria-hidden="true">{isOpen ? '−' : '+'}</span>
                </button>
